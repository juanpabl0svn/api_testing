...
router.get("/", (req, res) => {
  res.send(
    `<h1>Database test</h1><br/>Hello everyone,here if you want to see the register put in the path <a href="/api/db/data">/data</a>, if you want to filter the users by rol put <a href="/api/db/role/Student">/role/ROLEYOUWANT</a>, also you can authenticate here <a href='/api/db/auth/{"username":"juan123","password":"juanelmejor"}'>/auth/{"username":"YOURUSER","password":"YOURPASSWORD"</a> in JSON format, and finally if you want using the post request you can create your user here/create/"name":"YOURNAME","surname":"YOURSURNAME","role":"YOURROLE"`
  );
});
